...
public class BoardModify extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		if(request.getParameter("bno") != null
			&& Util.str2Int((String)request.getParameter("bno")) != 0) {
			
			int bno = Util.str2Int((String)request.getParameter("bno"));
			
			BoardDAO dao = BoardDAO.getInstance();
			HashMap<String, Object> list  = dao.detail(bno);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int bno = Util.str2Int(mpRequest.getParameter("bno"));
		...
		if (btitle.length() == 0 || bcontent.length() == 0) {
			...
			//id나 name이 없을 경우(로그인 에러) : code=511 다시 로그인 화면으로 보냅니다~
			...
			map.put("category", mpRequest.getParameter("category"));

			int result = BoardDAO.getInstance().modify(map, bno);
