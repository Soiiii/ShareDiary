...
</c:if> <!-- 총 갯수/10의 나머지가 0이 아니면 1페이지 추가 --> <!-- 위치변경했어요 -->
<c:if test="${page % PAGENUMBER ne 0 }"> <!-- page / PAGENUMBER로 되있던거 page % PAGENUMBER로 수정했어요 -->
	<fmt:parseNumber integerOnly="true" var="startPage" value="${page/PAGENUMBER }" scope="request"/> <!-- 위치변경했어요 -->
	<c:set value="${startPage * PAGENUMBER + 1 }" var="startPage" scope="request"/> <!-- 위치변경했어요 -->
</c:if> <!-- 11페이지가 넘어가면 startPage 재설정 --> <!-- 위치변경했어요 -->

<c:if test="${page % PAGENUMBER eq 0 }">
	<c:set value="${page - (PAGENUMBER - 1) }" var="startPage" scope="request"/>
</c:if>

<c:set value="${startPage + 9 }" var="endPage" scope="request"/> <!-- 위치변경했어요 -->
<!-- jstl 기호 참고) gt : >, lt : < -->
